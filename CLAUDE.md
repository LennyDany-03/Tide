...
- **The logo is `TideLogoPainter`** (`lib/widgets/tide_logo.dart`): ring, tide, moon, orbiting point, animated through a `TideLogoFrame`. `TideMark`, the splash and the platform app icons all draw from it. After changing it, regenerate the icons with `flutter test tool/brand_assets_test.dart` (kept out of `test/` because it writes into `android/`, `ios/` and `web/`).
